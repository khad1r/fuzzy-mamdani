...
class gayabelajar_model
{
    ...
    {
        $this->db->query('SELECT * FROM gayabelajar WHERE `sub-kriteria` IS NULL');
        $result = [];
    ...
    {
        $this->db->query('SELECT * FROM gayabelajar WHERE `sub-kriteria` IS NOT NULL');
        $result = [];
        foreach ($this->db->resultSet() as $value) {
            $result[$value['kode']] = $value;
        }
